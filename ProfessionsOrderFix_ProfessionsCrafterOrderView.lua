ProfessionsOrderFix_ProfessionsCrafterOrderViewMixin = {};
local ownReagentsConfirmationReferenceKey = {};
local ignoreConfirmationReferenceKey = {};

function ProfessionsOrderFix_ProfessionsCrafterOrderViewMixin:InitButtons()
...
function ProfessionsOrderFix_ProfessionsCrafterOrderViewMixin:InitRegions()
...
function ProfessionsOrderFix_ProfessionsCrafterOrderViewMixin:OnLoad()
    self:InitButtons();
    self:InitRegions();
...
function ProfessionsOrderFix_ProfessionsCrafterOrderViewMixin:OnEvent(event, ...)
...
function ProfessionsOrderFix_ProfessionsCrafterOrderViewMixin:OnShow()
...
function ProfessionsOrderFix_ProfessionsCrafterOrderViewMixin:ShowingGenericConfirmation()
...
function ProfessionsOrderFix_ProfessionsCrafterOrderViewMixin:CloseGenericConfirmation()
	if self:ShowingGenericConfirmation() then
		StaticPopup_Hide("GENERIC_CONFIRMATION");
	end
end

function ProfessionsOrderFix_ProfessionsCrafterOrderViewMixin:OnHide()
...
function ProfessionsOrderFix_ProfessionsCrafterOrderViewMixin:OnUpdate()
...
function ProfessionsOrderFix_ProfessionsCrafterOrderViewMixin:UpdateClaimEndTime()
...
function ProfessionsOrderFix_ProfessionsCrafterOrderViewMixin:CloseOrder()
    self:GetParent():CloseOrder();
end

function ProfessionsOrderFix_ProfessionsCrafterOrderViewMixin:CancelAsyncLoads()
...
function ProfessionsOrderFix_ProfessionsCrafterOrderViewMixin:SchematicPostInit()
	self:CancelAsyncLoads();

    -- We fix broken orders
    for _, value in pairs(recipesWithBrokenOrders) do
        if value == self.order.spellID then
            for _, reagent in pairs(self.order.reagents) do
...
function ProfessionsOrderFix_ProfessionsCrafterOrderViewMixin:UpdateMinimumQualityIcon()
...
function ProfessionsOrderFix_ProfessionsCrafterOrderViewMixin:UpdateStartOrderButton()
    local enabled = true;
    local errorReason;
...
function ProfessionsOrderFix_ProfessionsCrafterOrderViewMixin:UpdateFulfillButton()
    local enabled = true;
    local errorReason;
...
function ProfessionsOrderFix_ProfessionsCrafterOrderViewMixin:UpdateCreateButton()
    local transaction = self.OrderDetails.SchematicForm.transaction;
    local recipeInfo = C_TradeSkillUI.GetRecipeInfo(self.order.spellID);
...
function ProfessionsOrderFix_ProfessionsCrafterOrderViewMixin:UpdateRewards(order)
...
function ProfessionsOrderFix_ProfessionsCrafterOrderViewMixin:SetOrder(order)
    self.order = order;
...
function ProfessionsOrderFix_ProfessionsCrafterOrderViewMixin:SetOrderState(orderState)
...
function ProfessionsOrderFix_ProfessionsCrafterOrderViewMixin:CraftOrder()
...
function ProfessionsOrderFix_ProfessionsCrafterOrderViewMixin:RecraftOrder()
    self:SetOverrideCastBarActive(true);
    local predicate = function(reagentTbl, slotIndex)
		return reagentTbl.reagentSlotSchematic.dataSlotType == Enum.TradeskillSlotDataType.ModifiedReagent and not self.reagentSlotProvidedByCustomer[slotIndex];
	end
...
function ProfessionsOrderFix_ProfessionsCrafterOrderViewMixin:IsRecrafting()
    return self.order.isRecraft or self.recraftingOrderID == self.order.orderID
end

function ProfessionsOrderFix_ProfessionsCrafterOrderViewMixin:SetOverrideCastBarActive(active)
...
function ProfessionsOrderFix_ProfessionsCrafterOrderViewMixin:GetWhisperCustomerStatus()
	return self.whisperCustomerStatus;
end

function ProfessionsOrderFix_ProfessionsCrafterOrderViewMixin:SetWhisperCustomerStatus(status)
