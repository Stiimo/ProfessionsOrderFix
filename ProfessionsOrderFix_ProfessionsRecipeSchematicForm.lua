...
ProfessionsOrderFix_ProfessionsRecipeSchematicFormMixin = CreateFromMixins(CallbackRegistryMixin);

ProfessionsOrderFix_ProfessionsRecipeSchematicFormMixin:GenerateCallbackEvents(
{
...
function ProfessionsOrderFix_ProfessionsRecipeSchematicFormMixin:OnLoad()
	...
	self.AllocateBestQualityCheckbox:SetScript("OnClick", function(button, buttonName, down)
		...
		self:TriggerEvent(ProfessionsOrderFix_ProfessionsRecipeSchematicFormMixin.Event.UseBestQualityModified, checked);
...
function ProfessionsOrderFix_ProfessionsRecipeSchematicFormMixin:OnShow()
...
function ProfessionsOrderFix_ProfessionsRecipeSchematicFormMixin:OnHide()
...
function ProfessionsOrderFix_ProfessionsRecipeSchematicFormMixin:IsCurrentRecipe(recipeID)
	local currentRecipeInfo = self:GetRecipeInfo();
	local isCurrent = currentRecipeInfo and currentRecipeInfo.recipeID == recipeID;
	return isCurrent, currentRecipeInfo;
end

function ProfessionsOrderFix_ProfessionsRecipeSchematicFormMixin:OnEvent(event, ...)
...
function ProfessionsOrderFix_ProfessionsRecipeSchematicFormMixin:SetMaximized()
	self:Refresh();

	self.Details:SetMaximized();
end

function ProfessionsOrderFix_ProfessionsRecipeSchematicFormMixin:SetMinimized()
	self:Refresh();

	self.Details:SetMinimized();
end

function ProfessionsOrderFix_ProfessionsRecipeSchematicFormMixin:Update()
...
function ProfessionsOrderFix_ProfessionsRecipeSchematicFormMixin:Refresh()
	local recipeInfo = self:GetRecipeInfo();
	if recipeInfo then
...
function ProfessionsOrderFix_ProfessionsRecipeSchematicFormMixin:GetRecipeOperationInfo()
...
function ProfessionsOrderFix_ProfessionsRecipeSchematicFormMixin:GetOutputOverrideQuality()
...
function ProfessionsOrderFix_ProfessionsRecipeSchematicFormMixin:ClearTransaction()
...
function ProfessionsOrderFix_ProfessionsRecipeSchematicFormMixin:Init(recipeInfo, isRecraftOverride)
	local xPadding = 0;
	local yPadding = 4;
	...
	if newTransaction then
		self.transaction = CreateProfessionsOrderFix_ProfessionsRecipeTransaction(self.recipeSchematic);
					...
					if not slot:IsUnallocatable() and not self.isInspection then
						if buttonName == "LeftButton" then
							local function OnAllocationsAccepted(dialog, allocations, reagentSlotSchematic)
								...
								self:TriggerEvent(ProfessionsOrderFix_ProfessionsRecipeSchematicFormMixin.Event.AllocationsModified);
							end
...
function ProfessionsOrderFix_ProfessionsRecipeSchematicFormMixin:UpdateOutputItem()
	local reagents = self.transaction:CreateCraftingReagentInfoTbl();
...
function ProfessionsOrderFix_ProfessionsRecipeSchematicFormMixin:InitDetails(recipeInfo)
	local minimized = ProfessionsUtil.IsCraftingMinimized();
	local finishingSlots = self:GetSlotsByReagentType(Enum.CraftingReagentType.Finishing);
	local professionInfo = Professions.GetProfessionInfo();
	local operationInfo;
...
function ProfessionsOrderFix_ProfessionsRecipeSchematicFormMixin:OnAllocationsChanged()
	local operationInfo = self:GetRecipeOperationInfo();
...
function ProfessionsOrderFix_ProfessionsRecipeSchematicFormMixin:UpdateRecraftSlot(operationInfo)
...
function ProfessionsOrderFix_ProfessionsRecipeSchematicFormMixin:UpdateDetailsStats(operationInfo)
...
function ProfessionsOrderFix_ProfessionsRecipeSchematicFormMixin:ClearRecipeDescription()
	self.Description:SetText("");
	self.Description:SetHeight(1);
end

function ProfessionsOrderFix_ProfessionsRecipeSchematicFormMixin:UpdateRecipeDescription()
...
function ProfessionsOrderFix_ProfessionsRecipeSchematicFormMixin:SetSelectedRecipeLevel(recipeID, recipeLevel)
	self.selectedRecipeLevels[recipeID] = recipeLevel;
end

function ProfessionsOrderFix_ProfessionsRecipeSchematicFormMixin:GetSelectedRecipeLevel(recipeID)
	return self.selectedRecipeLevels[recipeID];
end

function ProfessionsOrderFix_ProfessionsRecipeSchematicFormMixin:GetTransaction()
	return self.transaction;
end

function ProfessionsOrderFix_ProfessionsRecipeSchematicFormMixin:GetRecipeInfo()
	return self.currentRecipeInfo;
end

function ProfessionsOrderFix_ProfessionsRecipeSchematicFormMixin:SetOutputSubText(text)
	self.OutputSubText:SetText(text);
	self.OutputSubText:Show();
end

function ProfessionsOrderFix_ProfessionsRecipeSchematicFormMixin:UpdateAllSlots()
	for slot in self.reagentSlotPool:EnumerateActive() do
		slot:Update();
	end
end

function ProfessionsOrderFix_ProfessionsRecipeSchematicFormMixin:GetSlots()
	local slots = {};
	for slot in self.reagentSlotPool:EnumerateActive() do
		table.insert(slots, slot);
	end
...
function ProfessionsOrderFix_ProfessionsRecipeSchematicFormMixin:GetSlotsByReagentType(reagentType)
	return self.reagentSlots[reagentType];
end

function ProfessionsOrderFix_ProfessionsRecipeSchematicFormMixin:GetCurrentRecipeLevel()
	return self.currentRecipeInfo and self:GetSelectedRecipeLevel(self.currentRecipeInfo.recipeID);
end

function ProfessionsOrderFix_ProfessionsRecipeSchematicFormMixin:OnHyperlinkEnter(link, text, fontString, left, bottom, width, height)
...
function ProfessionsOrderFix_ProfessionsRecipeSchematicFormMixin:OnHyperlinkLeave()
	GameTooltip_Hide();
end
